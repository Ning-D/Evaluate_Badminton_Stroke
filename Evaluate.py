...
files=file_list[split_index:]

number_of_total_game = len(files)
print("Test files: ",number_of_total_game)


number_of_total_game_train = len(file_list)
split_ratio = 0.95
split_index = int(number_of_total_game_train * split_ratio)

files_train=file_list[0:split_index]
#print(files_train)
number_of_total_game_train = len(files_train)

print("Train files: ",number_of_total_game_train)
...
def train_network(sess, model):
    ...
    for epcho in range(1,ITERATE_NUM+1):   
        ...
        saver.restore(sess, 'saved_models'+'_gamma'+str(GAMMA)+'_hd'+str(H_SIZE)+'_iter'+str(ITERATE_NUM)+'_lr'+str(learning_rate)+'/CNN_New'+str(epcho))
        ...
        AVG_MSEcost=Loss_total/float(number_of_total_game/BATCH_SIZE)
        ...
        print('AVG_MSEcost',AVG_MSEcost)
        
        game_diff_record_all.append(game_diff_record_dict)

    axs[0].plot(x_list,y_list,'o-',color='b',linewidth=1)    
    plt.savefig('Test_fn'+'_gamma'+str(GAMMA)+'_hd'+str(H_SIZE)+'_iter'+str(ITERATE_NUM)+'_lr'+str(learning_rate)+'.png')
